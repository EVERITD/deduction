...
/*error_reporting(0);*/
    ...
    $pbranch = htmlspecialchars(strip_tags($_GET['cmbranch']));
    $need = $_get['need'];    
...
// $pbranch = explode('-',$pbranch,-2);
    ...
    $pbranch = array_map('trim', $brd);   // trim the array
    $pbranch = array_filter($brd);
   /* echo*/ 
 ...
 $ctrlno2 = $_GET['chk2']; 
  ...
  while($cpRs = mssql_fetch_object($lRs))
  {
    ...
    $uniqueMainSites[] = trim($cpRs->main_site);
     ...
     if($_GET['active'] == 1)
     {
     include 'print_deduction_slip_view.php';
     }
     elseif($_GET['active'] == 2)
     {
        include 'print_deduction_slip_view2.php';
     }
     elseif($_GET['active'] == 3)
     {
        include 'print_deduction_slip_view3.php';
     }
