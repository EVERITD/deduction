...
include_once 'include_deduction_on.php';
...
// var cmbbranch = "<?php echo  $dept_code = $_SESSION['dept_code'];?>";
   	$(function() {
         ...
         if(cmbbranch != 'PUR' && cmbbranch != 'MKT' )
     	  {
							...
							?>
								<select name="branch" onChange="submit()" style="width: 90%;">
							  	...
							  	// 		if ($xbranch == 'S399') {
										...
										// 		and a.branch_code not in ('S801','S802','S803') order by a.branch_code";
										// }
										// $marker = 1;
								        if ($xbranch === 'S399') {
								            if ($_SESSION['dept_code'] === 'ACT') {
								    ...
								    if($marker AND trim($seluser) !== '')
								    {
											...
											//foreach($mSites as $v)
											//{
											//	$obSelected = ('OB-'.$v === trim($branch)) ? 'selected="selected"': '';
											//  echo "<optgroup label='Ordering Branch &mdash; ".$v."'>";
											//  echo "<option value='OB-{$v}' $obSelected>Ordering Branch &mdash; $v</option>";
											//  echo "<optgroup label='&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Receiving Branches'>";
												$selected = "";
												foreach($mData as $m)
												{
													$selected = "";
												...
												//	{
														echo "<option value='" . trim($m['branch_code']) . "' " . $selected . ">".$m['branch_code'].' &mdash; '.$m['branch_known_name'] . "</option>";
												//	}
												}
											//	echo "</optgroup>";
											//	echo "</optgroup>";
...
//                  					echo $stat;
                  	...
                  	<?php
                  	  ...
                  	  $isdeducted = $withcv->verifycv($d_row['branch_code'],$d_row['dmno']);

                  	  $_vposted = ($d_row['vposted'] === 4) ? 1: $d_row['vposted'];
                  	  $_deduct = ($d_row['deducted']);
                  	  if ($isdeducted === 0):
                  		if (in_array((int) $lcaccrights, array($_supervisor, $_encoder))):
                  			$_vpostedArr = array(1,2,3); // order => 1:approved, 2:cancelled, 3:extracted
                        	if (in_array((int) $_vposted, $_vpostedArr)):
							    if ((int) $lcaccrights === 4 and $_vposted == 1 and $_vposted == 0 AND trim($d_row['dmno']) === ''):
					          ?>
                                    <a href="dm_edit_on.php" class="ui-icon ui-icon-pencil" style="float: left; display: inline-block;" onClick="javascript:window.open('dm_edit_on.php?dmno=<?php echo trim($d_row['dm_no'])?>&branch=<?php echo trim($d_row['branch_code'])?>','','scrollbars=yes,resizable=no,width=400,height=510,left=200,top=95,dependent').focus();return false;" alt="Edit DM Deduction">Edit</a>
                          	...
                          	<?php
                                	...
                                	if((int) $_vposted === 1 AND ((int) $lcaccrights === 4)):
                        ...
                        ?>
                            <a href="dm_edit_on.php" onClick="javascript:window.open('dm_edit_on.php?dmno=<?php echo json_encode(array(trim($d_row['dm_no'])))?>&branch=<?php echo trim($d_row['branch_code'])?>','','scrollbars=yes,resizable=no,width=400,height=510,left=200,top=95,dependent').focus();return false;" alt="Edit DM Deduction">View</a>
                  	<?php 
                  		endif; 
                  	   endif;
                  	   include('sqlconn.php');
			...
			<tfoot>
				<tr>
					<td colspan="100%">
						...
						<a href = '#' style="float:right;">go to top &nbsp;&nbsp;&nbsp;</a>
					</td>
				</tr>
			</tfoot>
		</table>
