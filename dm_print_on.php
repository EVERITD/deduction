<?php
    ...
    $pbranch = htmlspecialchars(strip_tags($_POST['cmbbranch']));
    $need = $_get['need'];  
    // Old Code
    ...
    $brd = trim($_GET['branch']);
    $prt = $_GET['prt'];
  
    $ctrlno = $_POST['chk'];
    $ctrlll = @$_GET['ctrlno'];
    $ctrlno2 = json_encode($_POST['chk']); 
    ...
    if( ! $sqlCheckerId )
    {
        $dpssql = " select id from deduction_slip_prints
            where branch='{$pbranch}' and printed_date='{$prDate}' and print_by='{$lcuser}' ";
        ...
        else
        {
            ...
            $sqlInsert = " insert into deduction_slip_prints(branch, printed_date, print_by, created_at, reprint_at, deduction_slip_count, remarks, is_printed)
                values('{$pbranch}', '{$prDate}', '{$lcuser}', getdate(), NULL, ".count($ctrlno).", NULL, 0); select id from deduction_slip_prints
                where branch='{$pbranch}' and printed_date='{$prDate}' and print_by='{$lcuser}' ";
            $sqlPrintChecker = mssql_query($sqlInsert);
            $sqlCheckerId = mssql_result($sqlPrintChecker, 0, 'id');
        }
     ...
     * DM-Details
     */
        $sql = 'select deduction_master.*, (select branch_name from ref_branch where branch_code = \''.$pbranch.'\') as branch_name, ref_branch.branch_prefix,
            convert(char(8), deduction_master.dm_date, 112) as dm_date_int,
            convert(char(15), deduction_master.encoded_date, 121) as  fencoded_date,
            convert(char(15), deduction_master.review_date, 121) as  freview_date,
            ref_category.category_name, ref_branch.branch_code
            from deduction_master
            left join ref_branch on deduction_master.branch_code=ref_branch.branch_code
            left join ref_category on deduction_master.category_code=ref_category.category_code
            where deduction_master.paymentid = 2 and deduction_master.dm_no in (\''.$where.'\') :where1';

        // only $for _GET
        if(htmlspecialchars(strip_tags($_GET['ctrlno']), ENT_QUOTES))
            $sql = str_replace(':where1',' OR ltrim(rtrim(deduction_master.dm_no)) in ( \'' . $where1 . '\' ) ', $sql);

			$sql = str_replace(':where1', '', $sql.' order by deduction_master.dm_no_acctg ');
        ...
        while($rs = mssql_fetch_object($rst))
        {
            ...
            if($modified_rs->dm_no_acctg === ' ')
            {
                ...
                $qry = "update deduction_master set isforreview = 1,isposted = 1,isdmprinted = 1,dm_no_acctg='".$fmaxId."' where dm_no='".$rs->dm_no."'";
                $rs2 = mssql_query($qry);
            ...
            if(${$rs->dm_no}->dm_ctrl_no !== $rs->dm_no)
            {
                $sqls = " insert into deduction_slip_print_details( deduction_slip_prints_id, dm_ctrl_no, batch, created_at,
                          reprint_at, is_active, is_printed ) values ('{$sqlCheckerId}', '{$modified_rs->dm_no}', {$batchNo}, getdate(), NULL, 0, 0) ";
                mssql_query($sqls);
            }
  ...
  */
  ...
  $consPoSql = ' select * from conspo_pivot where dm_no in (\''.$where.'\') :where1 ';
  $consPoSql = str_replace('deduction_master.', '', $consPoSql);
  $consPoSql = str_replace(':where1',(htmlspecialchars(strip_tags($_GET['ctrlno']), ENT_QUOTES)) ? ' OR ltrim(rtrim(dm_no)) in ( \'' . $where1 . '\' ) ': '', $consPoSql);
  ...
  $lQry = " select a.*, b.branch_name from everlyl_conspo.consolidatepo.dbo.sitegroup a
