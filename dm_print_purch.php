...
if($need)
{
   ...
   $pbranch = htmlspecialchars(strip_tags($_GET['branch']), ENT_QUOTES);    
        ...
        if (str_word_count($pbranch)>1){
            $brCount = str_word_count($pbranch) - 1;
        } else {
             $brCount = str_word_count($pbranch);
        }
        $sbranch = explode('-',$pbranch,-$brCount);
...
$sqlQry1 = "select distinct deduction_slip_print_details.deduction_slip_prints_id as id,
    ...
    if( ! $sqlCheckerId )
    {
        ...
        else
        {
         ...
         $sqlInsert = " insert into deduction_slip_prints(branch, printed_date, print_by, created_at, reprint_at, deduction_slip_count, remarks, is_printed)
                values('{$singlebranch}', '{$prDate}', '{$lcuser}', getdate(), NULL, ".count($ctrlno).", NULL, 0)"; 
     ...
     */
       ...
       $sql = "select a.*, b.branch_name, b.branch_prefix,
            ...
            where a.paymentid in (1,2) and a.dm_no in ('{$where}') :where1";
...
if($os_platform ==  'Windows XP') 
{
      ob_start();
    include('dm_print_purch_view.php');
...
elseif($os_platform ==  'Windows 7' || $os_platform ==  'Windows 10')
{
