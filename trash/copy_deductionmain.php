...
<script type="text/javascript" language="javascript">
	...
	function ModalPopupsConfirm1() {
		ModalPopups.Confirm("idConfirm1",
			"System Message",
			"<div style='padding: 25px;'>You are about to cancel this control #.<br/><br/><b>Are you sure?</b></div>", 
			{
				yesButtonText: "Yes",
				noButtonText: "No",
	...
	function ModalPopupsConfirm2() {
		ModalPopups.Confirm("idConfirm1",
			"System Message",
			"<div style='padding: 25px;'>You are about to approve this control #.<br/><br/><b>Are you sure?</b></div>", 
			{
				yesButtonText: "Yes",
				noButtonText: "No",
	...
	function ModalPopupsConfirm3() {
		ModalPopups.Confirm("idConfirm1",
			"System Message",
			"<div style='padding: 25px;'>You are about to extract this control #.<br/><br/><b>Are you sure?</b></div>", 
			{
				yesButtonText: "Yes",
				noButtonText: "No",
				onYes: "ModalPopupsConfirmYes()",
		...
		<tr>
          <td height="490" valign="top" bgcolor="#ffffff">
			...
			?>
              ...
              <td height="28"><?php echo $d_row['dm_no']?></td>
			  ...
			  <td width="4%" align="center">
			  	<a href="dm_edit.php" onClick="javascript:window.open('dm_edit.php?dmno=<?php echo trim($d_row['dm_no'])?>&branch=<?php echo trim($d_row['branch_code'])?>','','scrollbars=no,resizable=no,width=400,height=410,left=200,top=95,dependent').focus();return false;" alt="Edit DM Deduction">Edit</a></td>
			  <?php 	}
			  		} else {?>
			  <td width="2%" align="center"><a href="dm_edit.php" onClick="javascript:window.open('dm_edit.php?dmno=<?php echo trim($d_row['dm_no'])?>&branch=<?php echo trim($d_row['branch_code'])?>','','scrollbars=no,resizable=no,width=400,height=410,left=200,top=95,dependent').focus();return false;" alt="Edit DM Deduction">Edit</a></td>
			  <?php }?>
            </tr>
			<?php
					}
	...
	if ($cmdextract != '') {
		if ($markall == "yes" ) {
			...
			if ($extno_nmrow == 0) {		
				...
				$update_xno = "Update dm_autoid set dm_no = '{$extno}' ";
				$rs_xno = mssql_query($update_xno);
			}
			...
			if (isset($_POST['box'])) {
				...
				if ($extno_nmrow == 0) {		
					...
					$zextno = 10 - (int)$yextno;
					$extno = str_pad(trim(substr($row_extno['extraction_refno'],0,-(int)$yextno)),$zextno,0).$xextno;
					echo $update_xno = "Update dm_autoid set dm_no = '{$extno}' ";
					$rs_xno = mssql_query($update_xno);
				}
