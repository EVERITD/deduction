...
$sqlQry1 = "select distinct deduction_slip_print_details.deduction_slip_prints_id as id,
     ...
     * DM-Details
     */
       $sql = 'select deduction_master.*, ref_branch.branch_name, ref_branch.branch_prefix,
            convert(char(8), deduction_master.dm_date, 112) as dm_date_int,
            convert(char(10), deduction_master.encoded_date, 101) as  fencoded_date,
      ...
      $sql = str_replace(':where1', '', $sql.' order by deduction_master.dm_no_acctg ');
       ...
       $rst = mssql_query($sql);
        ...
        while($rs = mssql_fetch_object($rst))
        {
            ...
            $ssql = "select aptid from eversql.arms.dbo.supplier where aptid='{$modified_rs->vendorcode}' OR vendorcode='{$modified_rs->vendorcode}' ";
