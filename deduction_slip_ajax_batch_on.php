<?php
    ...
    $bqry = " select distinct dspd.batch, dspd.deduction_slip_prints_id as dspid,dsp.print_by
					from deduction_slip_print_details dspd 
					left join deduction_slip_prints dsp on dspd.deduction_slip_prints_id=dsp.id 
					where :conspo :branch   convert(char(8),dsp.printed_date,112)='{$date}' ";
    ...
    $bqry = str_replace(':conspo', ($isCons) ? $consPo: '', $bqry);
    $bqry = str_replace(':branch', " branch like '{$branch}%' and ", $bqry);
    /*echo $bqry;*/
/*    echo $bqry;   S303 cases*/
/*echo $bqry; */
    $brst1 = mssql_query($bqry);
    $numrow = mssql_num_rows($brst1);
    if($numrow == 0)
    {
         $bqry = " select distinct dspd.batch, dspd.deduction_slip_prints_id as dspid,dsp.print_by
                    from deduction_slip_print_details dspd 
                ...
                if((int) $lcaccrights !== 4)
                    ...
                    $bqry .= " and dsp.print_by in (
                        select a.supervisor from ref_supervisor a, ref_users b where
                        a.supervisor='{$lcuser}' and a.user_name=b.user_name
                        and ltrim(rtrim(b.dept_code))='{$dept_code}' ) ";
                $bqry = str_replace(':conspo', ($isCons) ? $consPo: '', $bqry);
                $bqry = str_replace(':branch', " branch like '{$branch}%' and ", $bqry);
