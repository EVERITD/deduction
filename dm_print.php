<?php
     ...
     */
        $sql = 'select a.*, b.branch_name, b.branch_prefix,
            ...
            convert(char(15), a.encoded_date, 121) as  fencoded_date,
            convert(char(15), a.review_date, 121) as  freview_date,
            c.category_name, b.branch_code,d.buyer_code,ltrim(rtrim(a.department))+' - '+e.deptname as deptname
            ...
            left join ref_buyer d on a.buyerid = d.buyerid
        ...
        $sql = str_replace(':where1', '', $sql.' order by a.dm_no_acctg ');
        ...
        while($rs = mssql_fetch_object($rst))
        {
            ...
            if($modified_rs->dm_no_acctg === ' ')
            {
                ...
                $qry = "update deduction_master set isforreview = 1,isposted = 1,isdmprint = 1,dm_no_acctg='".$fmaxId."' where dm_no='".$rs->dm_no."'";
                $rs2 = mssql_query($qry);
