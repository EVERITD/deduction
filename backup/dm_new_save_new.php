...
<script type='text/javascript'>
  ...
  $(document).ready(function(){
    ...
    $('input#idlbrno').autocomplete("lbr_no_search.php", {
      matchContains: true,
      max: 0,
      minChars: 3
      })
    ...
    <td bgcolor="#ffffff"><select name="category_code" onChange="submit()" onkeypress="return ignoreenter(this,event)" style="width: 100%;" tabindex="3">
      ...
      <?php
          if ($lcdeptcode == 'PUR') {
            ...
            $selcat = "select category_code,category_name from ref_category where isactive = 1 and division_code = 'DS' order by category_name ";
          } else {
...
<tr>
    ...
    <td>LBR Number</td>
    <td>:</td>
    <td bgcolor="#ffffff"><input type="text" name="txtlbrno" maxlength="11" size="10" id="idlbrno"  value="<?php echo $txtlbrno ;?>" style="height: 18px; text-align:left;" tabindex="10" />  </td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td height="40">&nbsp;</td>
  ...
  <tr>
    ...
    <td>:</td>
    <td bgcolor="#ffffff"><textarea name="txtremarks" rows="3" cols="45" tabindex="12" onKeyPress="return imposeMaxLength(this, <?php echo (int)$remlenght - 1; ?>);"><?php echo $txtremarks ;?></textarea></td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td height="22">&nbsp;</td>
  ...
  if ($vcode_ischange == 'Approved') {
    if (strlen($category_code) > 0 and strlen($subcat_code) > 0 and $txtamount > 0 and strlen($vendor) > 0 and $ver_v > 0 and strlen($vcode) > 0 ) {
      ...
      $qryins = "Execute dm_insert '{$dmno}','{$branch}','{$div_code}','{$lcdeptcode}',
            ...
            '$txtamount','$payment','{$txtremarks}','{$lcuser}','{$lstbuyer}','{$txtlbrno}'";
      ...
      if($isConsoPo)
      {
        ...
        {
          echo $mQry = " delete from conspo_pivot where dm_no='$dmno'; insert into conspo_pivot (dm_no, main_site) values ('{$dmno}', '{$msRs->main_site}') ";
          mssql_query($mQry);
        }
        }
      }
