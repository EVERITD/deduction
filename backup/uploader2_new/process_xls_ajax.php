        ...
        foreach ($sheet as $row) {
            ...
            if ($counter !== 0 and ($row['A']
                ...
                if (trim(strtoupper($row['A'])) === 'S306')
                    $row['A'] = 'S399';
                $conso_br = funcGetConsoPO($row['A'], $row['H']);
                $dm_no = funcGetNewDm($row['A']);
                $suppliername = funcGetSupplierName($row['H']);
                $subcat = funcGetSubCat($row['E']);
                ...
                $qry = " insert into deduction_master(dm_no, branch_code, division_code, dept_code, category_code, subcat_code,promo, vendorcode, suppliername, dm_date,
                        period, amount, paymentid, remarks, remarks1, encoded_by, encoded_date, eposted, buyerid, isforreview, isposted, isdmprinted, LBR_Number)
                        values ( '{$dm_no}', '{$row['A']}', '{$row['B']}', '{$row['C']}',
                        '{$row['D']}', '{$row['E']}', '{$row['H']}', '" . trim(str_replace('\'', '\'+char(39)+\'', $suppliername)) . "',
                        getdate(), '{$row['I']}', '{$row['J']}', '{$row['K']}',
                        '{$subcat['shortcode']}', '{$subcat['longcode']}-{$row['L']}', '{$lcuser}',
                        getdate(), '{$id}', '{$row['G']}',1,1,1,'{$row['M']}')";
