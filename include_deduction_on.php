...
if(in_array(@$_POST['cmbstat'], array('0','1')) AND isset($_POST['cmbstat']))
{
  ...
  if($selstats !== '%')
    $printedIsApprove = " and a.deducted =".$selStat." ";
    if($selStat == "0") {
      $printedIsApprove = $printedIsApprove." and (j.aptno is null or j.aptno = '') ";
    }
...
switch($lststatus) {
  ...
  case "4": //for printed
    ...
    $qrydate = " convert(char(8),a.dm_date,112) as dm_date, dsp.printed_date, dspd.batch, ";
    $qrydmno = " a.dm_no_acctg as dmno, ";
...
else
{
  ...
  $select = " distinct a.dm_no,a.branch_code,c.dept_code,d.division_code,e.category_name,f.subcat_name,
    ltrim(rtrim(a.vendorcode))+' '+ltrim(rtrim(a.suppliername)) as suppliername,{$qrydate}{$qrydmno}
    a.amount,a.remarks,isnull(a.cancel_remarks,'') as cancel_remarks,a.encoded_by,review_by,
    ...
    isnull(h.buyer_code,'') as buyer_code,ltrim(rtrim(a.department))+ ' - '+ltrim(rtrim(k.deptname)) as department,isnull(i.paymentdesc,'') as paymentdesc,a.period,convert(char(8), a.review_date, 112) as review_date,
